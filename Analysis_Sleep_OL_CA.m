...
fileSleepDur = fopen([initPath.Exp 'data\OL_CA\group\sleepDuration.csv'],'w');
fprintf(fileSleepDur,'%s; %s; %s; %s\n', 'Sub' , 'Stage','DV','Percentage');

fileSleepLat = fopen([initPath.Exp 'data\OL_CA\group\sleepLatency.csv'],'w');
fprintf(fileSleepLat,'%s; %s; %s; %s; %s\n', 'Sub' , 'S1','S2', 'S3', 'REM');

fileSleepArousal = fopen([initPath.Exp 'data\OL_CA\group\sleepArousal.csv'],'w');
fprintf(fileSleepArousal,'%s; %s; %s; %s\n', 'Sub' , 'Nb','perHour','Duration');

fileStimEff= fopen([initPath.Exp 'data\OL_CA\group\stimEfficiency.csv'],'w');
fprintf(fileStimEff,'%s; %s; %s; %s; %s\n', 'Sub' , 'type','Stage','DV','Percentage'); 

for idx_sub = 1: length(listSub)
    
    sub = listSub{idx_sub};
    fasstFile     = [initPath.Exp '\data\OL_CA\' sub '\exp\' sub '.mat'];
    load(fasstFile)
    
    
    
        scoreFile     = [initPath.Exp '\data\OL_CA\' sub '\exp\FASST\' sub '_ExtraitEvts.csv'];  
        ...
        for idx = 1 : length(score.Groupe)
            ...
            if strcmp(score.Groupe(idx,1:2),'St')
                scoreSleep (counterStade,1) = score.Point_0x28Sec0xC9ch0x29(idx);
                scoreSleep (counterStade,2) = scoreSleep(counterStade,1)+score.Dur0xE9e_0x28Points0x29(idx)-1;
                
                if counterStade==1
                    scoreSleep(counterStade,1) =1;
                    scoreSleep(counterStade,2) =29999;
                end
    ...
    save([initPath.Exp '\data\OL_CA\' sub '\exp\' sub '_scored_epoch.mat'],'scoreSleep') % to be used in processed_continuous for detection
    ...
    load ( [initPath.Exp '\data\OL_CA\' sub '\exp\' sub '_trlRaw.mat'])
    ...
    stimStage = zeros(size(trl,1),2); %preallocate for speed
    for i = 1 : size(trl,1)
        clear epoch
    ...
    trl(:,5)= stimStage(:,1);
    save ([initPath.Exp '\data\OL_CA\' sub '\exp\' sub '_trl_epoch.mat'],'trl')
    ...
    WakeStim            = nnz(stimStage(:,1) == 0 & stimStage(:,2) == 1 );
    ...
    WakeRandom          = nnz(stimStage(:,1) == 0 & stimStage(:,2) == 0 );
    ...
    save( [initPath.Exp '\data\OL_CA\' sub '\exp\' sub '_sleepStats.mat'],'sleepStats')
end
